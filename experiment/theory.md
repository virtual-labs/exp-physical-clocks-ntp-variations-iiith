...
2. **Timestamps**:
   - NTP uses four timestamps in each exchange between a client and a server:
     - **T1**: The time at which the request is sent by the client.
     - **T2**: The time at which the request is received by the server.
     - **T3**: The time at which the server sends the response.
     - **T4**: The time at which the response is received by the client.
...
3. **Statistical Filtering**:
   - NTP does not rely on a single time measurement but uses multiple measurements over time to improve accuracy. Statistical filtering involves collecting multiple offset and delay samples, then applying algorithms to filter out anomalies and compute a reliable estimate of the time.
